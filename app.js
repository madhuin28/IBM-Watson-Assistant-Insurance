...
if (cloudantUrl) {
  ...
  // secure)
  nano.db.get('insurance_logs', function(err) {
    if (err) {
      console.error(err);
      nano.db.create('insurance_logs', function(errCreate) {
        console.error(errCreate);
        logs = nano.db.use('insurance_logs');
      });
    } else {
      logs = nano.db.use('insurance_logs');
    }
  ...
  app.post('/clearDb', auth, function(req, res) {
    nano.db.destroy('insurance_logs', function() {
      nano.db.create('insurance_logs', function() {
        logs = nano.db.use('insurance_logs');
      });
    });
